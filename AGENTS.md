...
- Every outside call (OpenAI, Google, SerpApi, page fetch, Wayback, Redis) goes through the outside-world boundary, which has `live` and `replay` modes. Replay is the default in dev and tests and costs $0; use live only when a ticket says so.
